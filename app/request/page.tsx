...
/**
 * Тип состояния для контактной информации пользователя.
 */
type ContactState = {
  name: string;
  email: string;
...
/**
 * Возможные состояния отправки формы. С помощью этих состояний
 * можно отображать сообщение об ошибке, процессе отправки или успехе.
 */
type SubmitStatus =
  | { state: "idle" }
  | { state: "sending" }
...
/**
 * Ключ для сохранения данных контакта в localStorage. Используется, чтобы
 * восстанавливать введённые значения между посещениями страницы.
 */
...
export default function RequestPage() {
  ...
  // При монтировании восстанавливаем контактные данные из localStorage,
  // если они были сохранены ранее.
  useEffect(() => {
  ...
  // Сохраняем контактные данные в localStorage при каждом изменении.
  useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(CONTACT_STORAGE_KEY, JSON.stringify(contact));
  }, [contact]);

  // Суммируем количество товаров по единицам измерения.
  ...
  // Подсчитываем количество пользовательских (кастомных) позиций.
  const customItemsCount = useMemo(
    () => items.filter((item) => item.custom).length,
    [items],
  );

  // Обработчик изменения количества. Преобразует ввод в число с учётом дробной части.
  ...
  // Отправка заявки на сервер. Выполняет валидацию перед отправкой.
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok || !json.ok) {
        throw new Error(
          json.message ?? "Не удалось отправить заявку.",
        );
      }
      setStatus({
        state: "success",
  ...
  // Скачивание коммерческого предложения в формате PDF.
        ...
        {isEmpty ? (
          <div className="rounded-3xl border border-white/15 bg-white/10 p-10 text-center text-sm text-white/80 shadow-[0_20px_60px_rgba(10,20,45,0.45)] backdrop-blur-xl">
            <p>
              ...
              <Link
                href="/catalog"
                className="font-semibold text-white hover:text-teal-200"
              >
                {"каталог"}
              </Link>{" "}
                          ...
                          {item.custom ? (
                            <div className="space-y-2">
                              ...
                              <p className="text-[11px] text-white/60">
                                {"Например: Сопутствующая услуга или вспомогательный товар"}
                              </p>
                            </div>
                          ) : (
                      ...
                      <div className="grid gap-3 md:grid-cols-[repeat(6,minmax(0,1fr))]">
                        ...
                        <label className="md:col-span-3">
                          ...
                          <input
                            value={item.comment ?? ""}
                            onChange={(event) =>
                              updateItem(item.id, { comment: event.target.value })
                            }
                ...
                <details className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/75 backdrop-blur">
                  ...
                  <label className="mt-3 block">
                    ...
                    <textarea
                      ...
                      placeholder="Опишите сроки, адрес и особенности задачи"
...
// Форматирование вывода количества в зависимости от точности (количества дробных знаков).
