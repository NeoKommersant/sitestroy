...
export default function Header() {
  ...
  const mobileTopBar = useMemo(
    () => (
      <div className="fixed top-0 left-0 right-0 z-40 flex items-center justify-center border-b border-transparent bg-slate-950/85 px-4 py-0 text-white backdrop-blur md:hidden">
        <Link href="/" className="flex items-center gap-1 text-white">
          <Image src="/img/logo.svg" alt="Логотип ГК «Строй Альянс»" width={40} height={40} className="h-10 w-10" priority />
          <span className="text-xs font-semibold uppercase tracking-[0.16em] text-white/80">ГК «Строй Альянс»</span>
        </Link>
      </div>
    ),
    [],
